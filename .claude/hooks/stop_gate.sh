...
if ! fired validate; then
  if ! (cd "$REPO" && timeout 300 make validate >/tmp/mhl-validate.log 2>&1); then
    R="make validate is RED (rule 4):"$'\n'"$(grep -E 'FAIL|Failed|fatal|error|Error' /tmp/mhl-validate.log | head -8)"; mark validate
  fi
fi
if ! fired incident; then
  OPEN=$(grep -lE '^status:[[:space:]]*open' "$INV"/incidents/INCIDENT-*.md 2>/dev/null | xargs -r -n1 basename)
  [ -n "$OPEN" ] && { R="$R"$'\n'"open incident(s) (rule 2) — codify the parameter changes or close them: $OPEN"; mark incident; }
fi
