...
1. `cd ~/workspace/McHomeLab/ansible && ../.venv/bin/ansible-playbook site.yml -i ../../McHomeLab-Inventory/hosts.yml --check --diff` — summarise per host: tasks that would change, with the diff for templated files (redact any secret values).
