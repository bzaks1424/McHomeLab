...
expect "outside-repo cwd: lab mutation still denied" $G deny  "$(bo 'ssh media.michaelpmcd.com "docker restart x"')"
expect "outside-repo cwd: governance write denied"   $G deny  "$(bo 'echo x > /home/mmcdonnell/workspace/McHomeLab/.claude/settings.local.json')"
